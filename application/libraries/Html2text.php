 ...
 *                                                                       *
 * 18/01/07 Added replacement, for </li> with \n. (morty@gmx.net)        *
...
class Html2text
{
	 ...
	 */
	var $search = array(
/*001*/ "#\r#",                                    // Non-legal carriage return
        "#[\n\t]+#",                               // Newlines and tabs
        '#<script[^>]*>.*?</script>#i',            // <script>s -- which strip_tags supposedly has problems with
		'#<([^/>][^>]*)>\s#',								//Move Whitespaces outside tags;
		'#\s</([^>]*)>#',								//Move Whitespaces outside tags;
        '#<h[123][^>]*>\s*(.+?)\s*</h[123]>#ie',      // H1 - H3
        '#<h[456][^>]*>\s*(.+?)\s*</h[456]>#ie',      // H4 - H6
        '#<blockquote[^>]*>\s*(?:<p[^>]*>)?\s*(.+?)(?:</p>)?\s*</blockquote>#i', // Block-Quotes
        '#<p[^>]*>\s*(.+?)</p>#i',                 // <P>
        '#<br[^>]*>\s*#i',                         // <br>
        '#<b[^>]*>\s*(.+?)\s*</b>|<strong[^>]*>\s*(.+?)\s*</strong>#i', // <b/> / <strong />
/*010*/ '#<i[^>]*>\s*(.+?)\s*</i>|<em[^>]*>\s*(.+?)\s*</em>#i', // <i />  / <em />
		'#<strike[^>]*>\s*(.+?)\s*</strike>#i', // <strike />
        '#</?ul[^>]*>#i',                          // <ul> and </ul>
        '#</?ol[^>]*>#i',                          // <ol> and </ol>
        '#<li[^>]*>\s*(.+?)</li>#i',               // <li>
        '#<a href="([^"]+)"[^>]*>\s*(.+?)\s*</a>#ie', // <a href="">
        '/<hr[^>]*>\s*/i',                         // <hr>
        '#</?table[^>]*>#i',                       // <table> and </table>
        '#</?tr[^>]*>#i',                          // <tr> and </tr>
        '#<td[^>]*>\s*(.+?)\s*</td>#i',            // <td> and </td>
/*020*/ '#<th[^>]*>\s*(.+?)\s*</th>#i',            // <th> and </th>
        '#&nbsp;#ie',
        '#&quot;#i',
        '#&gt;#i',
        '#&lt;#i',
...
/*050*/ '/&#710;/',
	 ...
	 */
	var $replace = array(
/*001*/ '',                                     // Non-legal carriage return
        ' ',                                    // Newlines and tabs
        '',                                     // <script>s -- which strip_tags supposedly has problems with
		' <\\1>',								//Move Whitespaces outside tags;
		'</\\1> ',								//Move Whitespaces outside tags;
       	"ucwords(\"\n\n!! \\1\\2 !!\n\n\")",     // H1 - H3
        "ucwords(\"\n\n! \\1\\2 !\n\n\")",        // H4 - H6
        "\n\n  | \\1\n\n",                      // Block-Quotes
...
/*050*/ 'ˆ',
	...
	function _convert()
	{
		...
		if ( $this->width > 0 ) {
			$text = $this->smart_wordwrap($text, $this->width);
		}

		// Add link list after Wrapping (Thanks to Karsten Tinnefeld)
	 ...
	 * @param integer $linewidth The Linewidth
	 * @param string $linebreak The Character for the lineending
	 * @return The broken string
	 */
	function smart_wordwrap($str, $linewidth = 72, $linebreak = "\n") {
		
		$lines = explode($linebreak, $str);
	  
		$total = "";
		foreach ($lines as $line) {
			$length = $this->mb('strlen', $line);
			 
			// count starting whitespace
			$offset = $length;
			for($i = 0; $i < $length; $i ++){
				if($line{$i} != " "){
					$offset = $i;
			...
			$start = $this->mb('substr', $line, 0, $offset);
			$between = $start;
			 
			// bullet point lines start "\s+*\s", blockquote lines "\s+|\s"
			$bullet = false;
			if ($this->mb('substr',$line, $offset, 2) == "* ") {
				$offset   += 2;
				$start   .= "* ";
				$between .= "  ";
			} else if ($this->mb('substr', $line, $offset, 2) == "| ") {
				$offset   += 2;
				$start   .= "| ";
				$between  = $start;
			}
			...
			$resultline = $start;
			$resultline .= wordwrap(substr($line, $offset),	
									$linewidth - $offset,	
									$linebreak . $between);
